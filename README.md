...
Each ShieldCI Docker image is built with [SLSA Level 3](https://slsa.dev/spec/v1.0/levels) provenance generated by [`slsa-github-generator`](https://github.com/slsa-framework/slsa-github-generator). The provenance attestation is stored in the public [Rekor](https://rekor.sigstore.dev) transparency log and ties the artifact cryptographically to the `Richonn/ShieldCI` repository and the specific CI workflow that produced it.

**Verify integrity and authorship of a release:**
...
The `--source-uri` parameter confirms the artifact was built from the official `Richonn/ShieldCI` repository. The provenance certificate contains the exact workflow path (`.github/workflows/ci.yml`) and commit SHA, providing cryptographic proof of both integrity and author identity.
